...
(** --- Consts --- **)
(* increase the fov, and the upper player will be smaller than the lower *)
let fovY = 16.9
let zNear = 100.0
...
(** --- Functions --- **)
(* TODO: This is not exactly Camera related, but Screen related. Maybe this file
should be renamed to Screen.ml and all code related with screen stuff (including
the Camera) would be placed in here *)
...
(* TODO: The rest of the file is a single function; some refactoringis needed *)
let calculateCamera ~fovy ~fovx ~znear ~posBottomPlayer ~posTopmostPlayer 
    ~deltaCameraBackwards ~posBall ~xCamBehav  ~mustShowBottomCourtLine = 
  ...
  let delta =
    ...
    let zOpponentOrMe = min (min (abs_float posTopmostPlayer.z2) (courtHt2))
        (abs_float posBottomPlayer.z2  )
    in
    degToRad (deltaForward +. t *. zOpponentOrMe)
  in
  let zPlayerOrBall = 
    let deltaZ = 200.0 in
    ...
    let rec calcRec ~xg ~zn ~zc = 
      let calc xgg = 
        ...
        if x1g -. x18 > d then
          ...
          if modxc1 < modxc2 then	xc1 else xc2
      in
      let xc = calc xg in
      let iCanSeeTheCourtCorner = 
        let ang =
          ...
          and fromEyeToCorner = 
            ...
            vec2dSub courtCorner eye
          in
          smallestAngleBetween fromEyeToCorner fromEyeToLookat
        in
        ang < fovXHalf 
      in
  ...
  let cameraZ ~alfa ~z3 ~z1 ~y1 ~delta = 
    let beta = pi -. alfa *. 0.5  -. delta in
    let eps = pi -. beta
    and c = 
      let gamma = pi -. alfa -. beta in
      let a = 
        let z0 = (* TODO: let z0 = z1 -. (y1 /. tan gamma) in *)
          let f = y1 /. tan gamma in
          z1 -. f in
        z3 -. z0 in
      (sin gamma) *. a /. (sin alfa)
    in
    let z4 = (* TODO: let z4 = z3 +. (c *. cos eps) in *)
      let g = c *. cos eps in
      z3 +. g 
    and zL = 
      let h = c *. sin (alfa *. 0.5) /. sin delta in
      z3 -. h 
    and y4 = c *. sin eps 
    in
    let d = y4 /. sin delta in
    (z4, zL, y4, d) (* TODO: (z4, zL, y4, y4 /. sin delta) *)
  in

  let (eyeZ, lookatZ, eyeY, d) = 
    let zOffsAboveHeadOfPAbove = 480.0 in
    cameraZ ~alfa:(fovy) ~z3:zPlayerOrBall 
      ~z1:(-. courtHt2) ~y1:zOffsAboveHeadOfPAbove ~delta
  in
  let (eyeX, eyeZ, lookatX) = 
    ...
    | BehindThePlayer ->
      let playerLine =
          straightLineBetween (vec2dCreate 0.0 (-. courtHt2)) posBottomPlayer
      in
      let horizLine = 
          straightLineBetween (vec2dCreate 0.0 eyeZ) (vec2dCreate 20.0 eyeZ) 
      in
    ...
    | PushScroll ->
      let xc, _, zc = 
        cameraX ~xg:posBottomPlayer.x2 ~zc:eyeZ ~zl:lookatZ
          ~zg:posBottomPlayer.z2  ~fovXHalf:(fovx *. 0.5) ~d:13.5
      in
      (xc, zc, 0.0)
  in
  { eyeX = eyeX; eyeY = eyeY; eyeZ = eyeZ; 
    lookatX = lookatX; lookatY = 0.0; lookatZ = lookatZ}
