...
let parseOptions=
  ...
  let arguments = [
    "-p0", Arg.Set_string p0Name,
      "The name of player 0. Possible names are \"pete\", \"ivan\", \"mats\".";
    "-p1", Arg.Set_string p1Name,
      "The name of player 1. Possible names are \"pete\", \"ivan\", \"mats\".";
    "-donotshowpause", Arg.Set doNotShowPause,
      "When the game is paused, do not show the \"paused\" sign. Useful for
      screenshots." ;
    "-showfps", Arg.Set showFps,
      "Display the number of Frames Per Second (on the standard output).
      Useful for debugging.";
    "-aidebug", Arg.Set aidebug, "Shows some info useful to debug AI.";
    "-realistic", Arg.Set realisticPar, "Make the game realistic. This alters
      the parabola visibility depending on the player and shot. Better gameplay,
      but not suitable for newbies.";
    "-newbie", Arg.Set newbiePar, "Make the game not realistic, but more suited
      to newbies. The parabola is fully visible.";
    ...
    "-port", Arg.Set_int port, "The TCP port to use. Makes sense with -client
      and -server. The default is " ^ string_of_int defaultPort;
    "-computerskill", Arg.Set_int computerSkill, "Skill level of computer.
      A number from 0 (very difficult) to 250 (very easy).
      Default is " ^ string_of_int defaultComputerSkill;
    "-client", Arg.Set_string client, "The IP address of the server. 
      Only for multiplayer.";
    "-server", Arg.Set server, "Run Free Tennis as server. Only for multiplayer.";
    "-surf", Arg.Set_string surf, "The surface for the court.
      Choices are \"clay\", \"cement\", \"grass\"."
  ]
  in
  let anonFun s = print_endline "No anonymous arguments allowed." in
  ...
  let parsedOptions = 
    if not !realisticPar && not !newbiePar then
      ...
      ArgumentError ("You cannot speficy both -realistic and -newbie. 
      ...
      match mayb with
      | None -> ArgumentError ("name " ^ !p0Name ^" invalid.")
      | Some p0 ->
        let mayb = translateName (String.lowercase !p1Name) in
        match mayb with
        | None -> ArgumentError ("name " ^ !p1Name ^" invalid.")
        | Some p1 ->
          let mayb = 
            let translateSurf s =
            ...
            in
            translateSurf (String.lowercase !surf)
          in
          match mayb with 
          | None -> ArgumentError ("surface " ^ !surf ^" is invalid.")
          | Some su ->
