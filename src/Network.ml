  ...
  let rec tryToConnectNTimes n ~soc ~inet_a ~port=
    ...
    with Unix.Unix_error _ ->
      if n = 0 then
        ...
        ( print_endline ("The server is down. Retrying " ^ string_of_int (n-1) 
                          ^ " times.");
          Unix.sleep 1;
    ...
    ( 
      ...
      (try
       ...
       with Unix.Unix_error (err, _, _) ->
         (print_endline ("Error: " ^(Unix.error_message err) ^ ". This is a
            known bug. Please wait a few seconds or simply change the
            port number with the -port option"); 
          exit 0 ));
      Unix.listen soc 5;
      print_endline "waiting for client to connect...";
      let clientSocket, _ = Unix.accept soc in
      Server ((soc, clientSocket), Unix.in_channel_of_descr clientSocket, 
              Unix.out_channel_of_descr clientSocket)
    )
