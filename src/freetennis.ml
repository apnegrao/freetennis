...
let _ = 
	...
	let parsedOptions, server, port, client = Options.parseOptions
    in
	...
	| ArgumentsOk opt ->
	      ...
	      let mouseSensitivity = 120.0  in	(**FIXME: move to Input.ml*)
		...
		source files, namely AnimationModule and *Player* *)
		  ...
		  let animdata = loadAnimData in
 		  let objLead =
			match serverData with	(*The matching results seem identical to me...*)
				| Client _ ->
