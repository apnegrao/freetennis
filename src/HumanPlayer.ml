...
let createHumanPlayer ~playsInTopmostCourtHalf ~plName ~scoreIndex ~startPos 
  ...
  { hp_objLeading = obLead;
    ...
    hp_maxParabOpacityGroundShots = 
      (match plName with Mats -> 0.069 | Ivan -> 0.054 | Pete -> 0.04);
    hp_maxParabOpacityVolleys = 
      (match plName with Mats -> 0.4 | Ivan -> 0.3 | Pete -> 0.5);
...
let calcMaxShotPower ~ballVelZ ~exploit ~search ~maxPow = 
  let po =
    match search with
    | RKH_Dive _ | RKH_StretchForward _ -> 
      maxPow *. powerAttenuationForStretchForwardAndDive
...
(** FIXME: This is the function that matters. However, it has over 2000 lines of
code, which makes it really difficult to understand...**)
(* TODO: Maybe start by moving the most important inner functions outside, maybe
...
@returns:	
*)
let updateHumanPlayer ~p ~dt ~b ~opponentCurPos ~mouse ~mouseSensitivity 
                      ~surf ~opt ~serverData =
  let dirsign = if p.hp_playsInTopmostCourtHalf then -. 1.0 else 1.0 
  and mouse2 =
  { mouse with 
      m_secondsSinceLastMouseMotion = mouse.m_secondsSinceLastMouseMotion +. dt
  }
  in
  let findNeededPower ~impact ~htOverNet ~spin ~bounceDesired = 
    ...
    let pointOverNet =
      ...
      intersectionOfStraightLines  netLine shotDir
    in
    (* cosi' rendo q0 positivo, quindi mi restringo al caso in cui gioca in basso *)
    let q0 = distance2d impact2d pointOverNet  in
    (* idem qui, che rendo qr negativo *)
        ...
        | Some pow ->
          if pow > maxPowerAvailable then
            ...
            ( print_endline "rejected at 2nd level because too weak";
              ( match reject with
                | None -> (* @@ happened playing volee with mats against ivan *) 
                  assert false
                | Some r -> r ))
          else
            (aim, ht)
      ...
      | Some isNotTooVert ->
        if isNotTooVert then Some bou
          ...
          let newCandidateBounceLonger = 
            ...
            vec2dAdd bou deltaXZ
          in
          shortestBounceSuchThatAngleIsNotTooVert ~bou:newCandidateBounceLonger
            ~count:(count -1) ~impact ~htOverNet ~spin
  in
  ...
  let rec highestHtSuchThatTheAngleIsNotVertAndPowerIsNotTooSmall ~ht ~count
    ...
    if count = 0 then None 
      ...
      | Some isNotTooVert ->
        if isNotTooVert then
          let maybePow = 
            findNeededPower ~impact ~htOverNet:ht ~spin ~bounceDesired:aim
          in
          match maybePow with
          | None -> None
          | Some pow ->
  ...
  let rec lowestHtSuchThatTheParabolaIsPossible ~ht ~count ~impact ~desiredAim 
    ...
    if count = 0 then None (* happens on dive *)
      ...
      in
      match maybePow with
      | None -> lowestHtSuchThatTheParabolaIsPossible ~ht:(ht +. 1.0)
                  ~count:(count -1) ~spin ~impact ~desiredAim 
      | Some _ -> (* parabola possible *) Some ht
  in
  ...
  let createStateASBO ~p ~r  =
    ...
    and maxPowerAvailable = calcMaxShotPower ~search:r.brh_researchKind 
        ~maxPow:p.hp_pc.pc_maxShotPower
        ~exploit:p.hp_pc.pc_exploitationOfOpponentsPower
    ...
    let mAimAndHt = 
      ...
      let maybeHigherHt = 
        let idealHtOverNet = 
          ...
          | RKH_Normal vt ->
            (match vt with
             | VOT_Volee -> p.hp_startHtOverNetForVolleys 
        ...
        lowestHtSuchThatTheParabolaIsPossible ~ht:idealHtOverNet ~count:1000
          ~impact:r.brh_impact ~desiredAim:idealAim ~spin
      in
      ...
      | Some higherHt ->
        ...
        and neededPowerTooLittle, neededPowerTooMuch =
          let mP = findNeededPower ~impact:r.brh_impact 
              ~spin ~htOverNet:higherHt ~bounceDesired:idealAim
          in
          match mP with
          | None -> assert false (* we know it's not impossible *)
        ...
        if angleTooVertical then
          let mLongerBounce = shortestBounceSuchThatAngleIsNotTooVert
              ...
              ~impact:r.brh_impact ~spin ~htOverNet:higherHt
          in
          match mLongerBounce with
          | None -> None (* @@ happened once. ivan at net *) 
          | Some longerBounce ->
            Some (doAllTests ~impact:r.brh_impact ~spin ~ht:higherHt 
                    ~aim:longerBounce ~minPowerAvailable
          ...
          | None -> None
          | Some lowerHt ->
            Some (doAllTests ~impact:r.brh_impact ~spin ~ht:lowerHt ~aim:idealAim
                    ~minPowerAvailable ~maxPowerAvailable ~reject:None)
        else if neededPowerTooMuch then
          let mShorterBounce = longestBounceSuchThatPowerIsNotTooMuch
              ~bou:idealAim ~count:1000 ~impact:r.brh_impact ~spin
              ~htOverNet:higherHt ~maxPowerAvailable
          in
          match mShorterBounce with
          | None -> None
          | Some shorterBounce ->
            Some (doAllTests ~impact:r.brh_impact ~spin ~ht:higherHt
              ~aim:shorterBounce ~minPowerAvailable ~maxPowerAvailable
              ~reject:None)
        else
  ...
  (** I think the functions so far serve to calculate the initial trajectory of
  the ball (i.e., before the user's input is taken into account. Based on 
  its name, this next one seems to be the one that adjusts the height of the
  trajectory over the net based on the user's mouse-controlled input  **)
    ...
    if up then
      ...
      if desiredHt < 10.0 then 
        ...
        let necessaryPower = findNeededPower ~impact
            ~htOverNet:desiredHt ~spin ~bounceDesired
        in
        ...
        | Some pow ->
          ...
          let mShorterBounce = longestBounceSuchThatPowerIsNotTooMuch 
              ~bou:bounceDesired ~count:1000 ~maxPowerAvailable ~impact ~spin
              ~htOverNet:desiredHt
          in
          match mShorterBounce with
          | None -> rejected
  ...
  let updateAimByReadingInput ~p ~dt ~idealHtOverNet ~impact ~spin
      ~maxPowerAvailable ~curAim ~mouse ~mouseSensitivity ~iCanIncreaseTheHt
      ~minPowerAvailable = 
    ...
    if dx = 0.0 && dz = 0.0 || 
        mouse.m_secondsSinceLastMouseMotion > mouseRefresh then
      updateRejected
    else
      ...
      if desiredAim.z2 *. impact.z3 >= 0.0 then
        ...
        let minDistanceOfBounceFromNet = 42.0 in
        if p.hp_playsInTopmostCourtHalf && 
            desiredAim.z2 < minDistanceOfBounceFromNet then
          updateRejected
        else if (not p.hp_playsInTopmostCourtHalf) && 
                desiredAim.z2 > -. minDistanceOfBounceFromNet then
          updateRejected
        else
          let theShotWasShortened =
            let imp = projection2d impact in
            length2d (vec2dSub curAim imp) > length2d (vec2dSub desiredAim imp)
          in
          if theShotWasShortened then
            (* the needed power may have become too much or too little; the angle
              may have become too vertical *)
            let maybePow = findNeededPower ~impact ~htOverNet:idealHtOverNet 
                ~spin ~bounceDesired:desiredAim
            in
            match maybePow with
            ...
            let maybeNewHt =
                lowestHtSuchThatThePowerIsNotTooMuchAndTheParabolaIsPossible
                  ~impact ~spin ~count:1000 ~maxPowerAvailable ~desiredAim
                  ~ht:idealHtOverNet
            in
  ...
  let p = 
    ...
    {p with hp_objLeading = newObjLead ; hp_objSlave = newObjSlave}
  in
  let judgeImpactPoint ~isVolley ~attackIntention ~opponentCurPos 
    ...
    if attackIntention then
      ( qual, (AI.voteImpactHtTheHigherTheBetter impact p.hp_pc.pc_maxSmashHt )
        *. 0.2 +. (AI.voteClosenessToNet footTarget) *. 0.7
        +. (AI.voteRunSpeed runSpeed myMaxSpeed) *. 0.1 )
    else
      ( qual, (AI.voteImpactHtCloseTo 170.0 impact p.hp_pc.pc_maxSmashHt) *. 0.5
  ...
  let newState, newObjLead2, newObjSlave2, newBall, newMouse, fatigueData', soundIds =
      ...
      let judgeAllResearches ~p ~isTopSpin ~playerCurPos ~opponentCurPos
          ...
          if impactIsOutSideOfMyCourtSide then
            ...
            | Some iaba ->
              ...
              if isVolley && s.bsm_lastShotWasAService then
                ...
                let footPosImpSmash =
                    footPosAtImpactTime ~deltaXFootRacket:0.0 ~deltaZ:80.0 in
                ...
                let deltaOpeningSmash =  computeDeltaOpening ~isForehand
                    ~researchKind:(RKH_Smash isVolley)
                in
                ...
                let calcVoteAndResearch ~researchKind ~tChange ~v2 ~footPosImp
                      ~runAngle ~signX ~deltaX=
                  let qual, vote = 
                    judgeImpactPoint ~attackIntention ~isVolley ~playerCurPos
                      ~opponentCurPos ~impact:impact3d ~footTarget:footPosImp
                      ~runSpeed:(if v2 < 0.0 then 0.0 else v2)
                  in
                  let speedBefOpening = 
                    if v2 < 0.0 then
                ...
                let canDoSmash = 
                  p.hp_pc.pc_minSmashHt < impact3d.y3 
                  && impact3d.y3 < p.hp_pc.pc_maxSmashHt && tChangeSmash >= 0.0
                  && deltaT >= deltaOpeningSmash &&  v2Smash <= speedNormalResearch
                in

                if canDoSmash then
                  calcVoteAndResearch ~researchKind:(RKH_Smash isVolley)
                    ~v2:v2Smash ~tChange:tChangeSmash ~footPosImp:footPosImpSmash
                  ...
                  let v2NormalShot = (deltaXNorm +. speedFreeRun
                                    *. (t0-. tChangeNormalShot))
                                    /. ( t1 -. tChangeNormalShot)
                  in
                  let canDoNormalShot = 
                    0.0 < impact3d.y3 && impact3d.y3 < p.hp_pc.pc_maxShotHt &&
                    tChangeNormalShot >= 0.0 && 
                    deltaT >= deltaOpening && v2NormalShot <= speedNormalResearch
                  in
                  if  canDoNormalShot then
                    calcVoteAndResearch ~researchKind:(RKH_Normal vot)
                      ~runAngle:runAngleNorm ~signX:signXNorm ~v2:v2NormalShot
                      ~tChange:tChangeNormalShot ~footPosImp:footPosImpNorm
                      ~deltaX:deltaXNorm
                  else
                    (* try stretch forward *)
                    let footPosImpStr = footPosAtImpactTime
                        ~deltaXFootRacket:(50.0) ~deltaZ:301.0
                    in
                    ...
                    let deltaOpeningStr =  computeDeltaOpening 
                        ~isForehand  ~researchKind:(RKH_StretchForward isVolley)
                    in
                    let tChangeStr = t1 -. deltaOpeningStr in
                    assert (tChangeStr <= t1);
                    let v2Str = (deltaXStr +. speedFreeRun 
                                *. (t0 -. tChangeStr) ) /. ( t1 -. tChangeStr)
                    in
                    let canDoStr = 0.0 < impact3d.y3
                      && impact3d.y3 < p.hp_pc.pc_maxShotHt && tChangeStr >= 0.0
                      && deltaT >= deltaOpeningStr && v2Str <= speedNormalResearch 
                    in
                    if canDoStr then
                      calcVoteAndResearch 
                        ~researchKind:(RKH_StretchForward isVolley) ~v2:v2Str
                        ~tChange:tChangeStr ~footPosImp:footPosImpStr
                        ~runAngle:runAngleStr ~signX:signXStr ~deltaX:deltaXStr
                    else
                      (* try dive *)
                      let footPosImpD = footPosAtImpactTime
                          ~deltaXFootRacket:(271.0) ~deltaZ:20.0
                      in
                      ...
                      let deltaOpeningDive =  computeDeltaOpening 
                          ~isForehand  ~researchKind:(RKH_Dive isVolley)
                      in
                      let tChangeDive = t1 -. deltaOpeningDive in
                      assert (tChangeDive <= t1);
                      let v2Dive = (deltaXD  +. speedFreeRun
                          *. (t0 -. tChangeDive) ) /. ( t1 -. tChangeDive)
                      in
                      let canDoDive = 0.0 < impact3d.y3 
                        && impact3d.y3 < p.hp_pc.pc_maxShotHt
                        && tChangeDive >= 0.0 && deltaT >= deltaOpeningDive
                        && v2Dive <= speedNormalResearch
                      in
                      if canDoDive then
                        calcVoteAndResearch ~researchKind:(RKH_Dive isVolley)
                          ~v2:v2Dive ~tChange:tChangeDive ~footPosImp:footPosImpD
                          ~runAngle:runAngleD ~signX:signXD ~deltaX:deltaXD
                      else
                        None
        ...
        | Some firstPointforResearch ->
          ...
          let impacts =
            List.map 
              ( fun fac ->
                vec2dAdd firstPointforResearch 
                  (vec2dMulScalar fac 
          ...
          in
          List.map 
            (fun x -> createResearchAndJudgeIt ~p ~isTopSpin ~impact:x
              ~playerCurPos ~ballPos ~ballDir ~ball ~s:bsm ~opponentCurPos
              ~surf ~attackIntention )
            impacts
      in (* judgeAllResearches *)

      let researches' = 
        let researches = judgeAllResearches ~p ~isTopSpin ~playerCurPos
            ~opponentCurPos ~ball ~ballPos ~ballDir ~bsm ~surf ~attackIntention
        in
        select_some researches
      in
      let smashVolleys =
        ...
        let voleesCloseToNet = 
          let isVoleeCloseToNet (res, _) =
            if abs_float res.brh_impact.z3 < courtHt4  then
              ...
              false
          in
          List.filter isVoleeCloseToNet researches'
        in
        ( match voleesCloseToNet with
          ...
          ( let normalShots =
            ...
            ( let smashGrounds = 
                ...
                in
                List.filter isSmashGround researches'
              in
              ...
              | [] -> 
                let stretches = 
                  let isStretch (res, _) =
                    ...
                    | RKH_Smash _ | RKH_Normal _ | RKH_Dive _ -> false
                  in
                  List.filter isStretch researches'
                in
                ( match stretches with
                  | [] -> 
                    let diveVolleys =
                      ...
                      in
                      List.filter isDiveVolley researches'
                    in
              ...
              | l -> Some (fst (findBestElement l better))
              (* end match smashGrounds *)
            )
            | l -> Some (fst  (findBestElement l better))
            (* end match normalShots *)
          )
          | l -> Some (fst (findBestElement l better))
        ) (* match voleesCloseToNet *)
      | l -> Some (fst (findBestElement l better))
      (* end match smashVolleys *)
    in
    ...
    let ifBallIsTowardsYouThenStartAutoSearch ~newObLead ~newObSlave ~newPos 
      ...
      then
        ...
        | Realized ->
          ...
          let doFakeDive ~reasonForMiss  = 
            ...
            let core = 
              if diveAngle < pi_2 then "rovescioallungov" else "drittoallungov"
            in
            let obl = 
            ...
            let umd = 
              let vx = 
                let lrSign = if diveAngle < pi_2 then -. 1.0 else 1.0 in
                lrSign *. dirsign
            ...
            ( HPS_DivingFake (umd, reasonForMiss), 
              obl,
              obs,
              SoundSprintJustBegun::newSounds
            )
          in
          match res with 
          | None ->
            let divePossib = 
              if diveHasEverBeenPossible = DivePossibilityUnknown then
                ...
                diveHasEverBeenPossible
            in
            if Sdlkey.is_key_pressed Sdlkey.KEY_d then
              doFakeDive ~reasonForMiss:(
                match divePossib with 
                | DiveHasNeverBeenPossible  -> DiveOnShotTooFar
          ...
          | Some r ->
            let onlyDive = 
              match r.brh_researchKind with
              | RKH_Dive _ -> true
              | RKH_Normal _ | RKH_StretchForward _ | RKH_Smash _ -> false
            in
            if not onlyDive &&  Sdlkey.is_key_pressed Sdlkey.KEY_d  then
              (* punished for diving without need *)
              doFakeDive ~reasonForMiss:DiveWithNoNeed
              ...
              let walkAni = 
                computeWalkAnim ~footTarget:r.brh_footTarget 
                  ~curPos:newPos ~dirsign
              in
              let newObLead = 
                let animName = prefixLead ^ walkAni in
                setAnim ~o:newObLead ~animName ~restartIfSameAnimation:false
              in
              let newObSlave = 
                let animName = prefixSlave ^ walkAni in
                setAnim ~o:newObSlave ~animName ~restartIfSameAnimation:false
              in
              ...
              | None ->
                (HPS_ManualSearch { 
                 ...
                 newSounds)
              | Some asb ->
                (HPS_AutoSearchBeforeOpening asb, newObLead, newObSlave, newSounds)
      else
        (HPS_ManualSearch { 
            hpsms_diveHasEverBeenPossible = diveHasEverBeenPossible;
            hpsms_pos = newPos;
          ...
          if newT < reflexDeltaT then
            ...
            let newst, newObLead, newObSlave, sounds =
              ifBallIsTowardsYouThenStartAutoSearch ~newObSlave:p.hp_objSlave
                ...
                ~newSounds:[] ~bsm ~realizationState:Realized
                ~newFatigueData:p.hp_fatigueData
                ~diveHasEverBeenPossible:DivePossibilityUnknown
                ~newSprint:HasNotAsked
            in
            (newst, newObLead, newObSlave, b, mouse, p.hp_fatigueData, sounds)
        | BS_Still _ ->
          ( HPS_ManualSearch
            { hpsms_pos = curPosOfHumanPlayer p;
              hpsms_realizing = Realized;
              hpsms_askedToSprintInPrevFrame =  HasNotAsked;
              hpsms_diveIsPossibleNow = DiveNotNeeded;
              hpsms_diveHasEverBeenPossible = DivePossibilityUnknown
    ...
    | HPS_AutoSearchBeforeOpening h ->
      let h = 
        ...
        | BS_Moving bsm ->
          ...
          if Sdlkey.is_key_pressed Sdlkey.KEY_z && backSpinMakesSense  then
            let attackIntention =
              abs_float (curPosOfHumanPlayer p).z2 < courtHt2 *. 3.0 /. 4.0
            in
      ...
      let newBounce, htOverNet = 
        ...
        let newBounce, htOverNet = updateHtOverNetByReadingInput ~dt ~p
            ~htOverNet:h.asbo_HtOverNet ~impact:h.asbo_Impact ~spin  
            ~bounceDesired:h.asbo_CurAim ~maxPowerAvailable
            ~minPowerAvailable ~mouse
        in
        updateAimByReadingInput ~p ~dt ~idealHtOverNet:htOverNet
          ~iCanIncreaseTheHt:true ~impact:h.asbo_Impact ~spin ~minPowerAvailable
          ~maxPowerAvailable ~curAim:newBounce ~mouse ~mouseSensitivity
      in
      ...
      if newT >= h.asbo_TimeToRunBeforeOpening then
        ...
        let nam = 
          ...
          | RKH_Dive _ ->
            if h.asbo_Forehand then "drittoallungov" else "rovescioallungov"
          | RKH_StretchForward _ ->
            if h.asbo_Forehand then "drittoforwardstretch"
            else "rovescioforwardstretch"
        in
        let newLead = 
          let animName = prefixLead ^ nam in
          setAnim ~o:p.hp_objLeading ~animName ~restartIfSameAnimation:true
        in
        let newSlave = 
          let animName = prefixSlave ^ nam in
          setAnim ~o:p.hp_objSlave ~animName ~restartIfSameAnimation:true
        in
        let sounds = 
          match h.asbo_researchKind with
        ...
        { h with 
          ...
          asbo_UniformMotionData = 
            { h.asbo_UniformMotionData with umd_timer = newT}
        }
        in
    ...
    | HPS_AutoSearchAfterOpening h ->
      ...
      let newBounce2, htOverNet2 = 
        ...
        in
        let newBounce, htOverNet = updateHtOverNetByReadingInput ~dt ~p
            ~htOverNet:h.asao_HtOverNet ~impact:h.asao_Impact ~spin 
            ~bounceDesired:h.asao_CurAim ~maxPowerAvailable ~minPowerAvailable
            ~mouse
        in
      ...
      if newT >= h.asao_TimeToRunFromOpeningToImpact then
        let maybeTr = 
          buildTrajFromTwoPointsAndHeight
            ...
            ~targetRect:(
              Some (
                if p.hp_playsInTopmostCourtHalf then lowerHalfOfCourt
                else upperHalfOfCourt))
        in
        match maybeTr with
        | None-> assert(false)
        | Some traj ->
          ...
          let newst =
            let dive =
              ( match h.asao_researchKind with
                ...
                | RKH_Smash _ | RKH_StretchForward _ | RKH_Normal _ -> false)
            in
            HPS_AutoSearchAfterImpactWaitingForAnimToEnd (umd, dive)
          in
          let pow = 
            let mpow = findNeededPower ~impact:h.asao_Impact ~spin
                ~bounceDesired:newBounce2 ~htOverNet:htOverNet2
            in
            match mpow with | None -> assert false | Some po -> po
          in
          print_endline
            ("human player -  vel z of opponent shot at impact: Kmh " ^
          ...
          let sounds = 
            ...
            | RKH_Dive _ -> [  SoundNormalShot]
            | RKH_Smash _ -> [  SoundNormalShot ; SoundAhh]
            | RKH_StretchForward _ -> [ SoundNormalShot ; SoundHff]
          in
          ( newst, p.hp_objLeading, p.hp_objSlave,
            bb, mouse2, p.hp_fatigueData, sounds )
      else
        let newst = 
          let hh =
          { h with
            ...
            asao_UniformMotionData =
              { h.asao_UniformMotionData with umd_timer = newT}
          }
          in
          HPS_AutoSearchAfterOpening hh
        in
        (newst, p.hp_objLeading, p.hp_objSlave, b, mouse2, p.hp_fatigueData, [])
    ...
    | HPS_DivingFake( u, reas) ->
      ...
      let newst = 
        if p.hp_objLeading.o3d_animState = NotAnimated then
          HPS_GettingUpAfterDive
          ( curPosOfHumanPlayer p, 0.0,
              match reas with
              | DiveTooLate -> true 
    ...
    | HPS_AutoSearchAfterImpactWaitingForAnimToEnd (u, dive) ->
      ...
      let newst = 
        if p.hp_objLeading.o3d_animState = NotAnimated then
          ...
          HPS_AutoSearchAfterImpactWaitingForAnimToEnd
            ({ u with umd_timer = t'}, dive)
      in
      ...
      let newPos, newFatigueData, newSprint, newSounds, newObLead, newObSlave = 
        ...
        | None ->
          let newObLead = 
            ...
            setAnim ~o:p.hp_objLeading ~animName ~restartIfSameAnimation:false
          in
          let newObSlave = 
            let animName = prefixSlave ^ "saltello" in
            setAnim ~o:p.hp_objSlave ~animName ~restartIfSameAnimation:false
          in
          ...
          let ds , fatigueData', askedSprintPrev', sounds = 
            ...
            if wouldLikeToSprint then
              let dsDesired = 
                let beta = 
                  if p.hp_playsInTopmostCourtHalf then
                    ...
                    smallestAngleWithNegativeZAxis velDir
                in
                dt *. sprintSpeed  ~beta ~speedFreeRun
              in
              ...
              let sou = 
                ...
                | HasNotAsked ->
                  (match maybe with
                   | None -> [SoundSprintCantBegin ]
                   | _ -> [SoundSprintJustBegun])
                | HasAskedAndWasDenied -> []
                | HasAskedAndObtained _ ->
                  (match maybe with
                   | None -> [SoundSprintJustFinished ]
                   | _ -> [])
              in
              match maybe with 
              | None -> (v *. dt, p.hp_fatigueData, HasAskedAndWasDenied, sou)
              | Some fatigueData' ->
          ...
          let newPos3 = 
            let newPos2 = 
              let newPos =
                ...
                vec2dCreate (m.hpsms_pos.x2 +. spostamentoX)
                            (m.hpsms_pos.z2-. spostamentoZ)
              in
      ...
      let newst, newObLead, newObSlave, sounds =
        ...
        | BS_Still _ ->
          ( HPS_ManualSearch
            ...
            newSounds)
        | BS_Moving bsm ->
          ifBallIsTowardsYouThenStartAutoSearch ~newObSlave ~newObLead ~newPos
            ~newSounds ~bsm ~realizationState:m.hpsms_realizing ~newFatigueData
        ...
        if p.hp_objLeading.o3d_animState = PausedDuringService then
          ...
          (HPS_ServingAfterLaunchAndBeforePressingButton (right, angle, pos),
           {mouse with m_secondsSinceLastMouseMotion = 
                          mouse.m_secondsSinceLastMouseMotion +.dt})
        else
          (HPS_ServingBeforeLaunch (right, pos), mouse)
      in
      let b2 = 
        if p.hp_objLeading.o3d_animState = PausedDuringService then
          let traj =
          { impact = vec3dCreate pos.x2 171.0 (dirsign*.(courtHt2 +. 20.0));
            ...
            targetRect = None}
          in
          ( createRunningBall ~traj ~scoreIndexOfLastPlayerWhoHit:b.b_siolpwhtb
    ...
    | HPS_ServingAfterLaunchAndBeforePressingButton (right, ang, pos) ->
      let state2, objLead2,objSlave2, b2 =
        ...
        let ballIsGoingDownOrStill = 
          match b.b_state with
          | BS_Still _ -> true
          | BS_Moving bsm -> (curBallVel bsm).y3 < 0.0
        in
        if bally < 170.0 && ballIsGoingDownOrStill  then
          let state2, objLead2, objSlave2 = 
              startServiceHuman ~scoreIsEven:right ~h:p ~serverData in
          ...
          in
          let oLead2 =
          { p.hp_objLeading with
              o3d_animState = Animated 0.0;
              o3d_curFrameIdx = p.hp_objLeading.o3d_curFrameIdx +1}
          in
          let oSlave2 =
          { p.hp_objSlave with
              o3d_animState = Animated 0.0;
              o3d_curFrameIdx = p.hp_objSlave.o3d_curFrameIdx +1}
          in
          ...
          ( HPS_ServingAfterLaunchAndBeforePressingButton (right, ang, pos),
            ...
            b)
      in
      let mouse2 = {mouse with m_secondsSinceLastMouseMotion =
                    mouse.m_secondsSinceLastMouseMotion +. dt }
      in
          ...
          let traj =
            let angX = 
              if h.hpssapb_FirstService then
                ...
                degToRad p.hp_pc.pc_secondServiceXAngleInDeg
            in
            let velservizio = 
              if h.hpssapb_FirstService then
                cmPerSecondOfKmh p.hp_pc.pc_firstServiceSpeedKmh
              else
                cmPerSecondOfKmh p.hp_pc.pc_secondServiceSpeedKmh
            in
  ...
  { newPlayer with
    hp_fatigueData = 
      (updateFatigue newPlayer.hp_fatigueData (curPosOfHumanPlayer newPlayer))
  }
  in
