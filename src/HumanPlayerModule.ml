...
(** This is the function that matters. However, it has over 2000 lines of code, which makes it really
...
@returns:	
*)
let updateHumanPlayer  ~p  ~dt ~b ~opponentCurPos  ~keyboard ~mouse ~mouseSensitivity 
			~surf ~serverData  ~remoteSign =
	...
	(**This looks like a relevant function, based on the arguments...**)
		    ...
		    | Realized ->
			  ...
			  match res with 
			      ...
			      | Some r ->
				    ...
				    if not onlyDive &&  Sdlkey.is_key_pressed Sdlkey.KEY_q  then
					doFakeDive ~reasonForMiss:DiveWithNoNeed (* punished for diving without need *)
				    else if onlyDive && not (Sdlkey.is_key_pressed Sdlkey.KEY_q) then
	...
	(** Is this the entrypoint of the function? I think so. Identifies the current
	state of the player and acts accordingly. **)
	match p.hp_state with
		  ...
		  let h = 
			  ...
			  | BS_Moving bsm ->
				let backSpinMakesSense  = 
					...
					| RKH_Smash _ -> false
				in
				if Sdlkey.is_key_pressed Sdlkey.KEY_SPACE && backSpinMakesSense  then
				    let attackIntention =
		...
		(** I think this is where the mouse controlled movement of the player is treated **)
	    | HPS_ManualSearch m  ->

		  let newPos, newFatigueData, newSprint, newSounds, newObLead, newObSlave = 

		      let wouldLikeToSprint =  Sdlkey.is_key_pressed Sdlkey.KEY_LSHIFT in
		      ...
		      in*)
			let maybeVelDir = 
				let keyboardVec, isPressed =
					let dz, isPressedVertical = 
						if keyboard.upKeyPressed then
							(remoteSign *. -.10.0, true)
						else if keyboard.downKeyPressed then
							(remoteSign *. 10.0, true)
						else
							(0.0, false)
					and dx, isPressedHorizontal = 
						if keyboard.leftKeyPressed then
							(remoteSign *. -.10.0, true)
						else if keyboard.rightKeyPressed then
							(remoteSign *. 10.0, true)
						else
							(0.0, false)
					in
					(*(normalize2d (vec2dCreate dx dz ), isPressed) *)
					if isPressedVertical || isPressedHorizontal then
						(normalize2d (vec2dCreate dx dz ), true)
					else
						(normalize2d (vec2dCreate dx dz ), false)
				in
				let maybeDxDz =
					if isPressed then
						Some keyboardVec
